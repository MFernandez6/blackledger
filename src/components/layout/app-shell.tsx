...
export function AppShell({ children, user }: AppShellProps) {
  ...
  return (
    <div className="min-h-dvh bg-brand-navy text-brand-white">
      <header className="no-print sticky top-0 z-50 border-b border-brand-white/5 bg-brand-navy/95 backdrop-blur-md">
        ...
        <nav className="flex gap-5 overflow-x-auto overscroll-x-contain border-t border-brand-white/5 px-4 py-3 lg:hidden">
          {nav.map((item) => (
            <Link
              key={item.href}
              href={item.href}
      ...
      <main
        className={cn(
          "mx-auto max-w-[1400px] min-w-0 px-4 py-6 animate-fade-in sm:px-6 sm:py-8",
